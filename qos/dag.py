...
from qos.virtual_gates import VIRTUAL_GATE_TYPES

class DAG(nx.DiGraph):
    def __init__(self, qiskit_qc: QuantumCircuit = None):
        #circuit = circuit.copy()
        if qiskit_qc is not None:
            def _next_op_on_qubit(qubit: int, from_idx: int) -> int:
                for i, instr in enumerate(qiskit_qc[from_idx + 1 :]):
            ...
            for i, instr in enumerate(qiskit_qc):
            ...
            self._qregs = qiskit_qc.qregs
            self._cregs = qiskit_qc.cregs

        # For now we just support qiskit but here would be the other quantum circuit from other libraries that could be submited by the client
